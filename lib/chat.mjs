...
export async function chat(plan,ui,options={}){
  ...
  const prepared=await prepareLocalExecution(plan,ui,options);
  const {runtimeDirectory,modelContent,integrityVerifiedAt}=prepared;
  plan=prepared.plan;
  const directory=path.join(appHome(),'sessions',`${Date.now()}-${process.pid}`);await mkdirPrivate(directory);
  const job=path.join(directory,'session.json');await writeNew(job,{plan,runtimeDirectory,modelContent,integrityVerifiedAt,tokens:options.tokens??1024});
  ...
  const timer=setTimeout(stop,(options.seconds??(options.once?600:3600))*1000);timer.unref();process.once('SIGINT',stop);process.once('SIGTERM',stop);
  try{
    ...
    while(!exited){
      ...
      if(result.type==='error'){if(options.once)throw new ApertureError('GENERATION_FAILED',result.message);ui.say(`Generation stopped: ${result.message}`);}
