...
export function makeFabricPlan(census,model,policy={}){
  ...
  for(const node of census.nodes){
    ...
    for(const device of node.devices||[]){
      if(device.kind!=='nvidia'||device.externalGate)continue;
      const usable=Math.max(0,Math.min(device.freeBytes,device.totalBytes)-reservePerGpuBytes);
      eligible.push({nodeId:node.id,deviceId:device.id,memoryDomain:device.memoryDomain,name:device.name,totalBytes:device.totalBytes,freeBytes:device.freeBytes,usableBytes:usable,computeCapability:device.computeCapability??null,link:device.link??null});
    }
  }
  eligible.sort((a,b)=>b.usableBytes-a.usableBytes||a.nodeId.localeCompare(b.nodeId)||a.deviceId.localeCompare(b.deviceId));
  if(eligible.length<requiredGpuCount)fail('GPU_COUNT',`Requires ${requiredGpuCount} admitted NVIDIA memory domains; ${eligible.length} are presently eligible.`,holds);
