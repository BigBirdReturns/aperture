...
export function makeFabricPlan(census,model,policy={}){
  ...
  if(policy.deviceName!=null&&(typeof policy.deviceName!=='string'||!policy.deviceName))throw new ApertureError('FABRIC_POLICY','deviceName must be a non-empty exact device name when supplied.');
  ...
  for(const node of census.nodes){
    ...
    for(const device of node.devices||[]){
      ...
      if(policy.deviceName&&device.name!==policy.deviceName)continue;
  ...
  const selector=policy.deviceName?` matching ${policy.deviceName}`:'';
  ...
  if(policy.homogeneous===true&&selected.length===requiredGpuCount&&new Set(selected.map(row=>row.name)).size!==1)fail('HOMOGENEOUS',`The admitted placement requires ${requiredGpuCount} homogeneous NVIDIA devices.`,holds);
  ...
  const requirements={gpuCount:requiredGpuCount,deviceName:policy.deviceName??null,homogeneous:policy.homogeneous===true,reservePerGpuBytes,hostReserveBytes};
