...
import {makePlan,explain} from './plan.mjs';
...
export async function wizard(ui,options={},deps={}){
  ...
  ui.say('01  MACHINE\nRead-only CPU, RAM, GPU, free-space and runtime inspection. No personal-folder search, benchmarks, model loading or uploads.');
  ...
  ui.say(`\n${machine.cpu}\n${gib(machine.memory.availableBytes)} available RAM${machine.gpu.devices.length?`; ${machine.gpu.devices.length} NVIDIA GPU(s)`:''}`);
  ...
  let action;
  for(;;){action=(await ui.ask('Your choice:'))?.trim();if(!action||['1','2','3','4'].includes(action))break;ui.say('Choose 1, 2, 3 or 4, or press Enter.');}
  ...
  await executePlan(plan,ui,{...options,experiment:action==='3',chat:action==='1'},deps);return plan;
}
export async function executePlan(plan,ui,{experiment=false,chat:chatMode=false,...options}={},deps={}){
  ...
  return active.model.kind==='gguf'?runNative(active,ui,{...options,experiment}):runHf(active,ui,{experiment});
}
