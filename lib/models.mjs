...
export async function inspectLocal(source, choose) {
  ...
  if (!p.toLowerCase().endsWith('.gguf')) {
    ...
    const signature=await headerFile(p,4);
    if(signature.buffer.toString('ascii')!=='GGUF')throw new ApertureError('MODEL_FORMAT', 'Choose a GGUF file (including an existing engine blob), a safetensors checkpoint folder, or its config.json.');
  }
  const files = [];
