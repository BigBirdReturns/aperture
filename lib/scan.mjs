...
export function parseNvidia(text) {
  ...
  for(const line of text.trim().split(/\r?\n/)) {
    ...
    devices.push({index:Number(index),uuid,name:clean(name),totalBytes,freeBytes,driver,capacity:'DRIVER_REPORTED',memoryDomain:uuid,externalGate:/CMP/i.test(name),computeCapability:null,pcie:null});
  }
  return devices;
}
export function parseNvidiaDetails(text) {
  const details=[];
  for(const line of text.trim().split(/\r?\n/)) {
    const row=line.split(',').map(x=>x.trim());if(row.length!==5)continue;
    const [index,uuid,computeCapability,generation,width]=row;
    if(!/^\d+$/.test(index)||!/^GPU-[a-z0-9-]+$/i.test(uuid))continue;
    const cc=Number(computeCapability),gen=Number(generation),laneWidth=Number(width);
    details.push({index:Number(index),uuid,
      computeCapability:Number.isFinite(cc)&&cc>0?cc:null,
      pcie:{generation:Number.isFinite(gen)&&gen>0?gen:null,width:Number.isFinite(laneWidth)&&laneWidth>0?laneWidth:null}});
  }
  return details;
}
export function mergeNvidiaDetails(devices,details) {
  const byUuid=new Map(details.map(detail=>[detail.uuid,detail]));
  return devices.map(device=>{
    const detail=byUuid.get(device.uuid);
    return detail?{...device,computeCapability:detail.computeCapability,pcie:detail.pcie}:device;
  });
}
export async function scan({storage=appHome()}={}) {
  const startedAt=now();
  const [hardware,disk]=await Promise.all([platformInventory(),storageAt(storage)]);
  hardware.memory??={};hardware.errors??=[];
  let total=hardware.memory.totalBytes??os.totalmem(),available=hardware.memory.availableBytes??os.freemem(),basis=hardware.memory.basis??'OS physical memory';
  if(process.platform==='darwin') {
    const memory=await observeMacMemory({totalBytes:total,rawFreeBytes:available});
    total=memory.totalBytes;available=memory.availableBytes;basis=memory.basis;
    hardware.memory={...hardware.memory,...memory};
    if(memory.error){hardware.errors=[...hardware.errors,memory.error];if(hardware.status==='OBSERVED')hardware.status='PARTIAL';}
  }
  ...
  if(smi)try {
    ...
    if(r.code===0){
      devices=parseNvidia(r.stdout);gpuObservation='NVIDIA driver inventory';
      try {
        const detail=await command(smi,['--query-gpu=index,uuid,compute_cap,pcie.link.gen.current,pcie.link.width.current','--format=csv,noheader,nounits']);
        if(detail.code===0)devices=mergeNvidiaDetails(devices,parseNvidiaDetails(detail.stdout));
      } catch {}
    }
  }catch{}
  return {schema:'aperture-scan/1',startedAt,observedAt:now(),platform:process.platform,architecture:process.arch,
    ...
    memory:{totalBytes:total,availableBytes:available,allocationHeadroomBytes,commitHeadroomBytes,basis,
      rawFreeBytes:hardware.memory.rawFreeBytes??null,pressureFreePercent:hardware.memory.pressureFreePercent??null,
    ...
    notMeasured:['memory/storage/network throughput','GPU stability','model performance','NPU model compatibility','non-NVIDIA dedicated capacity before native backend discovery','GPU peer-to-peer bandwidth and NUMA locality'],
