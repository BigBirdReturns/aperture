...
export function makePlan(machine,model,options={}) {
  ...
  const p=basePlan({...selectedMachine,memory:{...machine.memory,availableBytes:headroom}},model,{...options,cpu:forceCPU||options.cpu});
  p.machine=machine;
  ...
  p.request.threads=options.threads??Math.max(1,Math.min(8,Math.floor(machine.logicalCpus/2)));
  if(requested==='vulkan'||requested==='auto'&&p.method.backend==='cpu'&&(machine.hardware?.graphics||[]).length) {
...
export function explain(plan) {
  ...
  return baseExplain(plan)+native+`CPU worker threads: ${plan.request.threads??'runtime default'}.\n`;
}
