...
import {installedCudaLibraries,cudaLibraryEnvironment} from './cuda-libraries.mjs';
...
export function nativeEnvironment(plan,base=process.env) {
  ...
  return plan.method.backend==='cuda'?cudaLibraryEnvironment(env,plan.method.cudaLibraryDirectory):env;
}
async function probe(directory,backend,{uuid,index,libraryDirectory}={}) {
  let env={...process.env,NODE_LLAMA_CPP_SKIP_DOWNLOAD:'true'};
  if(backend==='cuda'&&libraryDirectory)env=cudaLibraryEnvironment(env,libraryDirectory);
  delete env.GGML_VK_VISIBLE_DEVICES;
...
export function deviceBudget(machine,observation) {
  ...
  if(nvidia.some(d=>d.externalGate))throw new ApertureError('CAPACITY_GATE','This accelerator requires the existing capacity qualification gate; Vulkan does not bypass it.');
...
export async function bindNative(plan,directory,ui) {
  ...
  if(current==='cuda') {
    try {
      const observation=await probeInstalledCuda(directory,plan.method.gpu.uuid,ui);
      ui.say(`Native CUDA device: ${observation.names.map(clean).join(', ')}.`);
      return {...plan,method:{...plan.method,expectedDeviceNames:observation.names,nativeObservation:observation,
        cudaLibraryDirectory:observation.libraryDirectory??null,cudaLibrarySource:observation.librarySource??null}};
    }catch(e){
...
async function probeInstalledCuda(directory,uuid,ui){
  try{return await probe(directory,'cuda',{uuid});}
  catch(original){
    for(const candidate of await installedCudaLibraries()){
      let observation;
      try{observation=await probe(directory,'cuda',{uuid,libraryDirectory:candidate.directory});}
      catch{continue;}
      ui.say(`CUDA initialized using ${candidate.source} (ABI ${candidate.abi}) in ${clean(candidate.directory)}. This library path is applied only to Aperture child processes.`);
      return {...observation,libraryDirectory:candidate.directory,librarySource:candidate.source};
    }
    throw original;
  }
}
