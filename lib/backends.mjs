...
export function deviceBudget(machine,observation) {
  ...
  return {name,memoryDomain,freeBytes:free,totalBytes:raw.total,budgetBytes:budget,ramBudgetBytes:ramBudget,
...
export async function bindNative(plan,directory,ui) {
  ...
  return {...plan,beyondVram:plan.model.bytes>screen.budgetBytes,method:{...plan.method,backend:'vulkan',gpu:null,
    vulkanIndex:chosen.index,nativeDevice:screen,expectedDeviceNames:chosen.names,nativeObservation:chosen,
    gpuBudgetBytes:screen.budgetBytes,ramBudgetBytes:screen.ramBudgetBytes,
