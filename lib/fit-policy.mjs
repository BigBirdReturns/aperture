...
export function fitBudgets(plan) {
  ...
  return {
    ...
    vram: cpu ? 0 : bytes(plan.method.gpuBudgetBytes ?? shared, 'GPU budget'),
    shared: shared == null ? null : bytes(shared, 'shared-memory budget')
  };
}
...
export async function assessFit(insights, plan, nativeOptions) {
  ...
  return {status: selected ? 'FITS_ESTIMATE' : 'DOES_NOT_FIT_ESTIMATE', budgets, selected, nearest,
    ...
    basis: 'Pinned runtime V2 resource estimates, which may use its estimation fallback. Not a load, execution or performance result.',
    selection: 'Highest estimated fitting GPU layer count, not a speed optimum; explicit layer counts are preserved.'};
}
export function explainFit(fit, phase) {
  ...
  return [`Native fit ${phase}: ${fit.status}; context ${fit.context}; sequences ${fit.sequences}.`,
    ...
    `Estimated ${c.gpuLayers} GPU layers; RAM ${gib(c.ram)} / budget ${gib(fit.budgets.ram)}; GPU ${gib(c.vram)} / budget ${gib(fit.budgets.vram)}.`,
    ...(fit.budgets.shared == null ? [] : [`RAM and graphics share ${gib(fit.budgets.shared)}; they are not additive capacity.`]),
