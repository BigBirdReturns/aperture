...
export function redactScan(machine){
  ...
  return {
    ...
    cpu:{model:text(machine.cpu),logicalCpus:finite(machine.logicalCpus),topology:list(hardware.cpu).map(item=>({model:text(item.name),cores:finite(item.cores),threads:finite(item.threads)}))},
    memory:{
    ...
    graphics:list(hardware.graphics).map(item=>({name:text(item.name),driver:text(item.driver),status:text(item.status),memoryDomain:text(item.memoryDomain),capacityBytes:finite(item.capacityBytes)})),
    neural:{status:text(hardware.neuralStatus),devices:list(hardware.neural).map(item=>({name:text(item.name),status:text(item.status),runtime:text(item.runtime),memoryDomain:text(item.memoryDomain)}))},
    nvidia:list(machine.gpu?.devices).map(item=>({index:finite(item.index),name:text(item.name),driver:text(item.driver),totalBytes:finite(item.totalBytes),freeBytes:finite(item.freeBytes),capacity:text(item.capacity),externalGate:!!item.externalGate})),
    storage:{
