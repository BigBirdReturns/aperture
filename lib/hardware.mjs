...
$r=[ordered]@{}; $errors=@()
try {$r.cpu=@(Get-CimInstance Win32_Processor | Select-Object Name,NumberOfCores,NumberOfLogicalProcessors)} catch {$errors+='CPU enumeration unavailable'}
try {$r.ram=@(Get-CimInstance Win32_PhysicalMemory | Select-Object Capacity,ConfiguredClockSpeed,DeviceLocator)} catch {$errors+='DIMM enumeration unavailable'}
try {$r.memory=Get-CimInstance Win32_PerfFormattedData_PerfOS_Memory | Select-Object AvailableBytes,CommittedBytes,CommitLimit} catch {$errors+='Memory/commit observation unavailable'}
try {$r.display=@(Get-CimInstance Win32_VideoController | Select-Object Name,DriverVersion,PNPDeviceID,Status)} catch {$errors+='Display enumeration unavailable'}
try {$r.npu=@(Get-CimInstance Win32_PnPEntity | Where-Object {$_.PNPClass -eq 'ComputeAccelerator' -or $_.Name -match '\bNPU\b|Neural Processing|AI Boost|Ryzen AI|Hexagon'} | Select-Object Name,PNPClass,Status,PNPDeviceID)} catch {$errors+='NPU enumeration unavailable'}
...
try {$r.links=@(Get-CimInstance Win32_PnPEntity | Where-Object {$_.Name -match 'Thunderbolt|USB4 Host|USB4 Router'} | Select-Object Name,PNPClass,Status)} catch {$errors+='External link enumeration unavailable'}
...
export async function platformInventory() {
  try {
    if(process.platform==='win32')return normalizeWindows(await jsonCommand('powershell.exe',['-NoLogo','-NoProfile','-NonInteractive','-Command',WINDOWS_QUERY],25000));
