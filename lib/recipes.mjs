...
const REFERENCE_MEMORY_TOLERANCE = 0.95;
...
export function evaluateRecipe(machine, recipe) {
  ...
  if (selected.length >= requiredCount && physicalFloor !== null) {
    ...
    else if (Math.min(...totals) < physicalFloor) blockers.push(`The established working-set floor for this exact recipe is ${gib(physicalFloor)} per GPU; the smallest selected GPU has ${gib(Math.min(...totals))}.`);
    else matches.push(`Physical GPU memory meets the ${gib(physicalFloor)} established per-GPU floor.`);
  }

  const observedWorkingSet = finite(recipe.reference.observedWorkingSetBytes);
  if (selected.length >= requiredCount && observedWorkingSet !== null) {
    const totals = selected.map(device => finite(device.totalBytes));
    if (totals.some(value => value === null)) unknowns.push(`The source reports an approximately ${gib(observedWorkingSet)} per-GPU working set, but local GPU capacity was not fully observed.`);
    else if (Math.min(...totals) < observedWorkingSet) unknowns.push(`The source reports an approximately ${gib(observedWorkingSet)} per-GPU working set for this reference configuration. The smallest selected GPU has ${gib(Math.min(...totals))}; the source does not establish an exact lower floor, so Aperture holds rather than converting the approximate observation into a hard rejection.`);
    else matches.push(`Physical GPU memory is at least the source's approximately ${gib(observedWorkingSet)} per-GPU working-set observation.`);
  }

  const referenceMemory = finite(recipe.reference.perGpuMemoryBytes);
  if (selected.length >= requiredCount && referenceMemory !== null) {
    const totals = selected.map(device => finite(device.totalBytes));
    const referenceClassFloor = referenceMemory * REFERENCE_MEMORY_TOLERANCE;
    if (totals.some(value => value === null)) unknowns.push(`The source reference used the ${gib(referenceMemory)}-class GPU; local GPU capacity was not fully observed.`);
    else if (Math.min(...totals) < referenceClassFloor) unknowns.push(`The source reference used the ${gib(referenceMemory)}-class GPU. This source does not establish that the same recipe fits in ${gib(Math.min(...totals))}; Aperture must resolve the exact checkpoint and context before claiming a lower floor.`);
    else matches.push(`Per-GPU physical memory is in the source's ${gib(referenceMemory)} reference class.`);
  }
  ...
  if (selected.length >= requiredCount) {
    ...
    if (free.every(value => value !== null)) {
      ...
      if (physicalFloor !== null && minFree < physicalFloor) {
        availability = 'INSUFFICIENT_CURRENT_HEADROOM';
        cautions.push(`Hardware capacity may qualify, but the smallest currently free GPU headroom is ${gib(minFree)} versus the ${gib(physicalFloor)} established floor.`);
      } else if (observedWorkingSet !== null && minFree < observedWorkingSet) {
        availability = 'BELOW_SOURCE_REFERENCE_HEADROOM';
        cautions.push(`The smallest currently free GPU headroom is ${gib(minFree)} versus the source's approximately ${gib(observedWorkingSet)} working-set observation. This is availability evidence, not a physical-capacity rejection.`);
  ...
  return {
    ...
    claimBoundary: 'Hardware/recipe compatibility only. A source reference is not silently converted into a minimum. Approximate source working-set figures remain reference evidence unless an exact lower floor is separately established. No model quality, throughput, architecture support, or successful load is claimed until separately measured.'
  };
}
