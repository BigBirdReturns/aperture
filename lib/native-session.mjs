...
export async function generate(api,plan,{prompt,tokens=128,onText=()=>{},onStage=()=>{},signal}={}) {
  ...
  try {
    ...
    if(plan.nativeFit&&model.gpuLayers!==plan.nativeFit.selected.gpuLayers)throw new ApertureError('PLACEMENT_CHANGED','Loaded GPU layer count differs from the assessed placement.');
