...
async function supervised(exe,args,env,seconds) {
  return new Promise((resolve,reject)=>{
    const child=spawn(exe,args,{env,shell:false,detached:process.platform!=='win32',stdio:'inherit',windowsHide:true});
...
export async function refreshed(plan) {
  const machine=await scan({storage:process.cwd()});
  ...
  if(machine.memory.availableBytes<plan.method.ramBudgetBytes)throw new ApertureError('RAM_CHANGED','Available RAM is below the saved budget. Run setup again.');
  return {...plan,machine};
}
export async function runNative(plan,ui,{experiment=false,...options}={}) {
  ...
  for(let i=0;i<(experiment?2:1);i++){
    ...
    const env={...process.env,NODE_LLAMA_CPP_SKIP_DOWNLOAD:'true',CUDA_VISIBLE_DEVICES:plan.method.gpu?.uuid||''};
