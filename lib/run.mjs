...
export async function runNative(plan,ui,{experiment=false,...options}={},deps={}) {
  ...
  const {prepareLocalExecution}=await import('./native-fit.mjs');
  const prepare=deps.prepareLocalExecution??prepareLocalExecution;
  const launch=deps.supervised??supervised;
  ...
  for(let i=0;i<(experiment?2:1);i++){
    ...
    let prepared;
    try {prepared=await prepare(plan,ui,options);}
    catch(e){
      const detail={status:'REFUSED_BEFORE_LOAD',code:e.code??e.name,message:String(e.message)};
      trials.push({number:i+1,status:'REFUSED',result:detail});
      ui.say(`Trial ${i+1} refused before model loading: ${detail.code}: ${detail.message}`);
      break;
    }
    const {plan:trialPlan,runtimeDirectory,modelContent,integrityVerifiedAt}=prepared;
    await writeNew(jobFile,{plan:trialPlan,runtimeDirectory,modelContent,integrityVerifiedAt,prompt,tokens,resultFile});
    const env=nativeEnvironment(trialPlan);
    ui.say(experiment?`Experiment run ${i+1}/2:`:'Starting the selected model:');
    const result=await launch(process.execPath,[path.join(root,'lib','native-worker.mjs'),jobFile],env,seconds);
