...
`npm test` exercises 162 source/control cases. `node scripts/package-smoke.mjs` builds the package, installs it through an isolated npm cache, checks its actual version, runs the installed `aperture support` command, parses the reduced JSON, and rejects forbidden identifier fields and local host, home, or temporary-path values. The GitHub workflow runs those checks on Node 22 and 24 across Ubuntu, Windows and macOS. A green control job does not establish native inference on that hosted platform or anonymity of a support receipt.
...
## 0.4.5 support receipt and release candidate

Aperture 0.4.5 adds a separate `aperture-support/1` output generated from the approved hardware scan. It retains CPU topology, physical and available RAM, allocation and commit headroom where available, integrated and discrete graphics, NPU candidates, drivers, storage classes and capacities, external-link observations, runtime presence, scan status, and explicit unmeasured fields. It does not read a model, contact a model host, install a runtime, execute inference, stress hardware, or upload the output.

The reduction removes host and user names, local paths and mount labels, stable device and partition identifiers, GPU UUIDs, drive product names and serials, network adapter names and addresses, model locations, prompts, generated text, credentials, and environment variables. Dynamic provider errors are reduced to bounded classes. Labels shaped like PCI addresses, PnP identifiers, MAC addresses, or absolute paths are withheld. Six controls cover invalid input, identifier-shaped labels, retained hardware classes, denial before scanning, new-file-only output, and stdout output.

The installed-package smoke path builds the package, installs it through a fresh npm cache, executes `aperture support --allow-scan`, parses the resulting JSON, checks schema and version identity, rejects forbidden identifier fields, and rejects the runner's host name, home directory, and temporary path in every string value. The six-cell hosted matrix exercises that path on Node 22 and 24 across Ubuntu, Windows, and macOS. This qualifies the implemented reduction and command availability; it does not establish anonymity or numerical inference on the hosted runners.

The package-bearing source commit `4dc4def93b1a8a33fb57a4960d873e770c24455d`, tree `27038c3ecbf667e254e248093e0a741a008b61f3`, produced two byte-identical `bigbirdreturns-aperture-0.4.5.tgz` archives. Each archive is 72,691 bytes with SHA-256 `1b539bbfb6eeb315a9cbc341baf0d2978546106da3efce9ef1afd1ca43bae13e`. All 162 source/control tests and the installed-package support receipt passed in that candidate job. Publication must rebuild the package twice from an ancestor containing those exact package inputs and refuse any byte or digest difference.

The receipt remains fingerprintable through retained hardware models, driver versions, exact capacities, current headroom, scan duration, and its timestamp. It is classified `REDACTED_SUPPORT_RECEIPT_REVIEW_REQUIRED`, not anonymous. It does not extend the 0.4.4 numerical evidence, prove execution on a new person's rig, add NPU inference, add native Mac inference, or implement distributed execution. The control question is whether the reviewed receipt supplies enough machine evidence to reproduce a model-fit or backend failure while excluding the local identifiers and content that belong only in private custody.
