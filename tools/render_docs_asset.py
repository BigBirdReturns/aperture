...
markup = '<!doctype html><html lang="en"><meta charset="utf-8"><style>' + style + '</style><body><main class="canvas"><div class="top">' + mark_svg() + '<span>SANDHU CONSULTING GROUP<br>OPEN-SOURCE LOCAL INFERENCE</span></div><h1>APERTURE</h1><h2>Configure local inference around<br>your chosen model.</h2><p class="sub">Supply a link or local file. Inspect the hardware.<br>Review the configuration and open a supported local session.</p><div class="bottom"><span>bigbirdreturns.github.io/aperture</span><span>MODEL-FIRST LOCAL INFERENCE</span></div></main></body></html>'
