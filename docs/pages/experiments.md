...
```powershell
npx.cmd --yes --package=https://github.com/BigBirdReturns/aperture/releases/download/v0.4.4/bigbirdreturns-aperture-0.4.4.tgz aperture experiment answer.json
```

On Linux/macOS replace `npx.cmd` with `npx`. The controller asks for explicit experiment approval and any other permissions still required. Release 0.4.4 runs two bounded generations preserving the requested model and context, with fresh integrity and fit checks for each trial. A later refusal retains earlier results. It does not implement an automatic parameter sweep or change the model on your behalf.
...
Run files can contain prompts, output, paths, and device identifiers. Nothing is uploaded automatically. Release 0.4.4 has no automatic redaction/export command. Review and minimize a result before sharing it through an issue. See [privacy and local data](privacy.md) and [verified support](support.md).
