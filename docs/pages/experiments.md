...
```powershell
npx.cmd --yes --package=https://github.com/BigBirdReturns/aperture/releases/download/v0.4.1/bigbirdreturns-aperture-0.4.1.tgz aperture experiment answer.json
```

On Linux/macOS replace `npx.cmd` with `npx`. The controller asks for explicit experiment approval and any other permissions still required. Release 0.4.1 runs two bounded generations with the same configuration. It does not implement an automatic parameter sweep or change the model on your behalf.
...
Run files can contain prompts, output, paths, and device identifiers. Nothing is uploaded automatically. Release 0.4.1 has no automatic redaction/export command. Review and minimize a result before sharing it through an issue. See [privacy and local data](privacy.md) and [verified support](support.md).
