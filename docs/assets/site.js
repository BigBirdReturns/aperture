(() => {
  ...
  theme.addEventListener('change', () => {
    ...
    try { localStorage.setItem('aperture-appearance', theme.value); } catch { /* Storage is optional. */ }
  });
  const docnav = $('#docnav');
  ...
  if (install) {
    ...
    const update = () => {
      ...
      const suffix = distribution.value === 'git'
        ? `github:BigBirdReturns/aperture#${install.dataset.tag}`
        : `--package=${install.dataset.package} aperture`;
      ...
      $('.copy-status', install).textContent = '';
      $('.copy', install).textContent = 'Copy command';
    };
  ...
  document.querySelectorAll('[data-copy]').forEach(button => {
    button.addEventListener('click', async () => {
      ...
      const status = $('.copy-status', button.closest('.codebox'));
      try {
