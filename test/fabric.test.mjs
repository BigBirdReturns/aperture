...
const gpu=(id,domain,free=23,name='NVIDIA GeForce RTX 3090')=>({id,memoryDomain:domain,kind:'nvidia',name,totalBytes:24*GiB,freeBytes:free*GiB,computeCapability:8.6,externalGate:false,link:{kind:'PCIe',generation:4,width:16}});
const census=()=>({schema:'aperture-fabric-census/1',authorityNodeId:'head',adapters:[adapter],nodes:[
  {id:'head',state:'READY',memory:{allocationHeadroomBytes:64*GiB},devices:[gpu('gpu0','domain0','23','NVIDIA RTX 3090')]},
...
const policy={gpuCount:4,deviceModel:'RTX 3090',homogeneous:true,reservePerGpuBytes:2*GiB,hostReserveBytes:4*GiB,adapter:'llama.cpp-rpc',canaries:[2048,8192,32768],benchmark};
...
test('four 3090s remain four memory domains rather than one 96 GiB device',()=>{
  ...
  assert.equal(plan.requirements.deviceModel,'RTX 3090');
  assert.equal(plan.requirements.canonicalDeviceModel,'rtx 3090');
  assert.equal(plan.requirements.homogeneous,true);
  assert.match(plan.memory.statement,/not one addressable VRAM pool/);
});

test('NVIDIA and GeForce branding variants resolve to the same canonical 3090 model',()=>{
  const plan=makeFabricPlan(census(),model(),policy);
  assert.equal(plan.status,'READY_TO_CANARY');
  assert.deepEqual(new Set(plan.memory.gpuDomains.map(row=>row.canonicalModel)),new Set(['rtx 3090']));
});
...
test('canonical 3090 selector refuses another GPU model',()=>{
...
test('canonical 3090 selector does not silently admit RTX 3090 Ti',()=>{
  const c=census();c.nodes[2].devices[0]=gpu('gpu0','domain3',23,'NVIDIA GeForce RTX 3090 Ti');
  const plan=makeFabricPlan(c,model(),policy);
  assert.equal(plan.status,'HOLD');
  assert.ok(plan.holds.some(row=>row.code==='GPU_COUNT'));
});

test('homogeneous placement remains an independent policy when no model selector is supplied',()=>{
  const c=census();c.nodes[2].devices[0]=gpu('gpu0','domain3',23,'NVIDIA GeForce RTX 5090');
  const plan=makeFabricPlan(c,model(),{...policy,deviceModel:null});
  assert.equal(plan.status,'HOLD');
...
test('ordinary per-node Aperture scans become one fabric census without opening the occupied seat',()=>{
  const scan=(uuid,index=0)=>({schema:'aperture-scan/1',observedAt:'2026-09-07T00:00:00Z',memory:{totalBytes:64*GiB,allocationHeadroomBytes:48*GiB},gpu:{devices:[{index,uuid,memoryDomain:uuid,name:'NVIDIA GeForce RTX 3090',totalBytes:24*GiB,freeBytes:23*GiB,computeCapability:8.6,externalGate:false,pcie:{generation:4,width:16}}]}});
  const observations=[
